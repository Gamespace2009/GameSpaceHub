...
function card_on(el) {
    ...
    else if (el.id == "NovaBounce") {
        description.innerHTML = `Ты — космический защитник, и твоя задача — не дать сверхновой упасть в пустоту.<br> Управляй платформой, отбивай энергию и разрушай звёздные блоки.<br>Каждый сбитый блок приближает тебя к вспышке новой звезды.<br>Сможешь ли ты очистить всё поле?` 
        name_game.textContent = "Отскок Сверхновой"
        rules.innerHTML =  `Управляй платформой с помощью мыши (зажми левую кнопку и двигай).<br> Отбивай шарик так, чтобы он не упал вниз.<br>Разрушай все блоки, чтобы победить.<br>Если шарик падает — игра заканчивается.<br>Счётчик очков показывает, сколько блоков ты разрушил.`
    }

    else if(el.id == "mazespace") {
        description.innerHTML = `MazeSpace — это рогалик-лабиринт, где случай решает, как далеко ты сможешь пройти.<br>Ты оказываешься в центре космического лабиринта, где каждый шаг — это риск и выбор.<br>Система сама определяет, сможешь ли ты идти вперёд, или упрёшься в стену.<br>Твоя задача — найти выход, не потерявшись в бесконечных коридорах.`
        name_game.textContent = "космический лабиринт"
        rules.innerHTML = `Нажми «Направление» — система выберет направление: горизонталь или вертикаль.<br>    Выбери сторону: влево/вправо или вверх/вниз.<br>    Автоматически бросается 1–6 шагов — столько клеток ты пройдёшь.<br>    Если на пути стена — ты остановишься, не дойдя до конца.<br>    Если упёрся в стену — нажми «Реран», чтобы перебросить направление.<br>    Найди золотую клетку — это выход. Ты победил 🎉`
    }
